...
| `handoff_target` | `civil-materials-citation`, `civil-materials-figure`, `review_handoff`, or `table_system`. |
...
Before handing rows to `civil-materials-citation`, make sure every borrowable claim has:
...
Before handing rows to `civil-materials-figure`, make sure every figure-worthy row has:
