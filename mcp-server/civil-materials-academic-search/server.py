#!/usr/bin/env python3
"""FastMCP-based MCP server for civil-materials academic search."""
...
logger = logging.getLogger("civil-materials-academic-search")
service = AcademicSearchService()
mcp = FastMCP("civil-materials-academic-search")
...
def lookup_mesh(topic: str, limit: int = 10) -> str:
    """Look up PubMed MeSH terms for a civil-materials or chemistry topic."""
...
def export_citation_matrix(
    ...
    """Export rows compatible with the civil-materials citation matrix template."""
    args: dict[str, Any] = {"material_domain": material_domain}
    if topic:
        args["topic"] = topic
    if claims:
